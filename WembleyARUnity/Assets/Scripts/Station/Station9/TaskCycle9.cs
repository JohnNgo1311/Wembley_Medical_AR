...
public class TaskCycle9 : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if ((currentValue >= 920) || (currentValue >= 0 && currentValue < 120))
        {
        ...
        {
            if ((GlobalVariable.outputStation7[conditionIndex] == true || isOn == true) && currentValue >= minValue && currentValue < maxValue)
            {
                sliderBar.GetComponent<Slider>().value = currentValue;
                frameOn.SetActive(true);
                frameOff.SetActive(false);
