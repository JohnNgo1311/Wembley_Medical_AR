...
public class IOListS11 : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            if (GlobalVariable.inputStation11[i]) inputCheck[i].SetActive(true);
            else inputCheck[i].SetActive(false);
        }
        for (var i = 0; i < outputCheck.Length; i++)
        {
            if (GlobalVariable.outputStation11[i]) outputCheck[i].SetActive(true);
