...
public class IOListS12 : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            if (GlobalVariable.inputStation12[i]) inputCheck[i].SetActive(true);
            else inputCheck[i].SetActive(false);
        }
        for (var i = 0; i < outputCheck.Length; i++)
        {
            if (GlobalVariable.outputStation12[i]) outputCheck[i].SetActive(true);
