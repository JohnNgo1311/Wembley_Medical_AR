...
public class IOListS4 : MonoBehaviour
{
    ...
    //     {
    //         if (GlobalVariable.inputStation4[i]) inputCheck[i].SetActive(true);
    //         else inputCheck[i].SetActive(false);
    //     }
    //     for (var i = 0; i < outputCheck.Length; i++)
    //     {
    //         if (GlobalVariable.outputStation4[i]) outputCheck[i].SetActive(true);
